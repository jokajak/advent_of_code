...
#
# Your puzzle answer was 375054920.


def valid_list(input, preamble):
    """Determine if input is valid

    Args:
        input (list): The list of entries
        preamble (int): The length of the preamble
    """
    ret = False
    # Last entry is the target value
    expected_answer = input[-1]
    # Slice the list from the preamble size to the last value
    test_combinations = combinations(input[-(preamble+1):-1], 2)
    # Check all combinations
    for combo in test_combinations:
        if sum(combo) == expected_answer:
            ret = True
            break
    return ret


def find_invalid_entry(input, preamble):
    """Find an invalid entry in a ordered list

    Args:
        input (list): The ordered list of values
        preamble (int): The size of the preamble
    """
    slice_start = 0
    last_slice_start = len(input) - preamble
    while slice_start < last_slice_start:
        is_valid_value = valid_list(input[slice_start:slice_start+preamble+1], preamble)
        if not is_valid_value:
            return input[slice_start+preamble], slice_start+preamble
        slice_start += 1
...
def find_encryption_weakness(input, preamble):
    """Find the sum of the smallest and largest number in contiguous range

    Args:
        input (list): The input list
        preamble (int): The length of the preamble
    """
    # First find the invalid entry
    invalid_entry, invalid_entry_index = find_invalid_entry(input, preamble)
    # Because the values after the preamble grow due to the algorithm we know we don't need to parse anything after the
    # invalid entry position
    smaller_input = input[0:invalid_entry_index]
    start_index = 0
    while start_index < invalid_entry_index:
        stop_index = start_index + 1
        while stop_index < invalid_entry_index:
            if sum(smaller_input[start_index:stop_index]) == invalid_entry:
                min_val = min(smaller_input[start_index:stop_index])
                max_val = max(smaller_input[start_index:stop_index])
...
def test_valid_list(input, expected):
    input, preamble = input
    assert valid_list(input, preamble) == expected
...
def test_find_invalid_entry(input, expected):
    input, preamble = input
    assert find_invalid_entry(input, preamble) == expected
...
def test_find_encryption_weakness(input, expected):
    input, preamble = input
    assert find_encryption_weakness(input, preamble) == expected
...
def main(args):
    ...
    print(find_invalid_entry(input, 25))

    print(find_encryption_weakness(input, 25))
