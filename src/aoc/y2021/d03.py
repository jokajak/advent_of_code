#!/usr/bin/env python
"""AoC 3, 2021.
Created: 2022-11-30 22:40:27.312379
...
Considering only the first bit of each number, there are five `0` bits
 and seven `1` bits. Since the most common bit is `1`, the first bit
of the gamma rate is `1`.
...
gamma rate and epsilon rate, then multiply them together. *What is the
 power consumption of the submarine?* (Be sure to represent your
...
def solve_part_one(input_data):
    ...
    answer = int("".join(gamma_rate), 2) * int("".join(epsilon_rate), 2)
    return answer
...
def solve_part_two(input_data):
    ...
    answer = oxygen_generator_rating * carbon_dioxide_generator_rating
    return answer
