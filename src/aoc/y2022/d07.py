#!/usr/bin/env python
"""Solutions for AoC 7, 2022."""
# Created: 2022-12-07 08:56:50.467133

# Standard library imports
from aocd.models import Puzzle

from aoc.y2022.communications import Directory, walk_commands


def parse(input_data):
    """Transform the data"""
    res = walk_commands(input_data)
    return res


def get_contained_sizes(tree: Directory, max_size=100000):
    total_size = 0
    if tree.size < max_size:
        total_size += tree.size
    for entry in tree.children:
        if isinstance(entry, Directory):
            total_size += get_contained_sizes(entry, max_size)
    return total_size
...
def solve_part_one(input_data):
    ...
    directly or indirectly. (Directories themselves do not count as having
     any intrinsic size.)
    ...
    `f` (size 29116), `g` (size 2557), and `h.lst` (size 62596), plus file
    ...
    """
    answer = get_contained_sizes(input_data, 100000)
    return answer
...
def solve_part_two(input_data):
    ...
    used_space = total_space - input_data.size
    needed_space = target_free_space - used_space
    candidates = []

    def get_tree(tree):
        path_sizes = {}
        path_sizes[tree.path] = tree.size
        for entry in tree.children:
            if isinstance(entry, Directory):
                path_sizes.update(get_tree(entry))
                path_sizes[entry.path] = entry.size
        return path_sizes

    tree = get_tree(input_data)
    for path, size in tree.items():
        print(f"{path}: {size} needed: {needed_space} used: {used_space}")
        if size > needed_space:
            candidates.append(size)
    assert len(candidates) > 0
    smallest = sorted(candidates)[0]
    return smallest
