#!/usr/bin/env python
"""AoC 3, 2022.
Created: 2022-12-03 08:40:17.089756
...
def score(item):
    """Calculate the score of an item.
...
def solve_part_one(input_data):
    ...
    for line in input_data:
        line_len = len(line)
        left_compartment = line[0 : int(line_len / 2)]
        right_compartment = line[int(line_len / 2) :]
        common_item = set(left_compartment).intersection(right_compartment)
        assert len(common_item) == 1
        common_items.append(common_item.pop())
    answer = sum([score(item) for item in common_items])
    return answer
...
def solve_part_two(input_data):
    ...
    for index in range(0, len(input_data), 3):
        common_item = (
            set(input_data[index])
            .intersection(input_data[index + 1])
            .intersection(input_data[index + 2])
        )
        assert len(common_item) == 1
        common_items.append(common_item.pop())
    answer = sum([score(item) for item in common_items])
