#!/usr/bin/env python
"""Solutions for AoC 11, 2022."""
# Created: 2022-12-11 10:00:16.741185
...
class Item(object):
    """Simple class for items and worry levels"""

    def __init__(self, worry_level, nervous=True):
        self.worry_level = worry_level
        self.nervous = nervous

    def test(self, moduli):
        return self.worry_level % moduli
    ...
    def worry(self, operand, value):
        if value == "old":
            value = self.worry_level
        else:
            value = int(value)
        self.worry_level = operand(self.worry_level, value)
        self.worry_level = self.worry_level % total_modulus
    ...
    def __eq__(self, o):
        if isinstance(o, Item):
            print(
                f"{self.worry_level} == {o.worry_level} {self.worry_level == o.worry_level}"
            )
            return self.worry_level == o.worry_level
...
class Monkey(object):
    ...
    def __init__(self, action, value, moduli, true_monkey, false_monkey):
        self.action = action
        self.value = value
        self.items = []
        self.total_items_touched = 0
        self.moduli = moduli
    ...
    def inspect_items(self):
        ...
        worry level after the monkey inspected the item is five times whatever
        ...
        for item in self.items:
            self.total_items_touched += 1
            item.worry(self.action, self.value)
            item.relax()
    ...
    def throw_items(self):
        ...
        for _ in range(len(self.items)):
            ...
            ret.append((self.next_monkey(item), item))
        return ret

    def next_monkey(self, item):
        if item.test(self.moduli) == 0:
            return self.true_monkey
        else:
            return self.false_monkey

    @classmethod
    def read_lines(cls, lines, nervous=True):
        """Parse lines
        ...
        for line in lines:
            match line.strip().split(":", maxsplit=1):
                case ["Starting items", monkey_items]:
                    for e in monkey_items.split(","):
                        items.append(int(e.strip()))
                case ["Operation", operation_line]:
                    # Operation: new = old * 19
                    operation_line = operation_line.strip()
                    operand = operation_line.split(" ")[3]
                    value = operation_line.split(" ")[4]
                    match operand:
                        case "*":
                        ...
                        case _:
                            raise ValueError("Unknown operator: ", operand)
                case ["Test", test_line]:
                    test_line = test_line.strip()
                    moduli = int(test_line.split(" ")[2])
                case ["If true", line_action]:
                    line_action = line_action.strip()
                    true_monkey = int(line_action.split(" ")[3])
                case ["If false", line_action]:
                    line_action = line_action.strip()
                    false_monkey = int(line_action.split(" ")[3])
        monkey = cls(operand, value, moduli, true_monkey, false_monkey)
        for item in items:
            monkey.receive_item(Item(item, nervous))
        return monkey

    def __eq__(self, o):
        if isinstance(o, Monkey):
            print(f"{self.moduli} == {o.moduli} {self.moduli == o.moduli}")
            print(f"{self.value} == {o.value} {self.value == o.value}")
            print(f"{self.action} == {o.action} {self.action == o.action}")
            print(
                f"{self.true_monkey} == {o.true_monkey} {self.true_monkey == o.true_monkey}"
            )
            print(f"{self.false_monkey} == {o.false_monkey}")
            return (
                self.moduli == o.moduli
                and self.value == o.value
                and self.action == o.action
                and self.true_monkey == o.true_monkey
                and self.false_monkey == o.false_monkey
            )
        return NotImplemented
    ...
    def __str__(self):
        match self.action:
            ...
            case operator.mul:
                op = "*"
        res = f"""Starting items: {self.monkey_items}
    Operation: new = old {op} {self.value}
    Test: divisble by {self.moduli}
      If true: throw to monkey {self.true_monkey}
      If false: throw to monkey {self.false_monkey}"""
        return res


def parse(input_data, nervous=True):
    """Transform the data"""
    monkey_lines = []
    monkeys = []
    found_monkey = False
    for line in input_data.splitlines():
        ...
        if line.startswith("Monkey "):
            if found_monkey:
                monkeys.append(Monkey.read_lines(monkey_lines, nervous=nervous))
                monkey_lines = []
...
def solve_part_one(input_data):
    ...
    Away](https://en.wikipedia.org/wiki/Keep_away) with your missing
    things!
    ...
    *end* of the recipient monkey's list. A monkey that starts a round
    ...
    """
    monkeys = run_rounds(input_data)
    items_touched = sorted([monkey.total_items_touched for monkey in monkeys])
    print(items_touched)

    answer = items_touched[-2] * items_touched[-1]
    return answer


def run_rounds(input_data, rounds=20):
    monkeys = input_data
    for i in range(rounds):
        for i, monkey in enumerate(monkeys):
            monkey.inspect_items()
            next_monkeys = monkey.throw_items()
            for next_monkey_index, item in next_monkeys:
                # print(f"Monkey {i} passing {item} to {next_monkey_index}")
                monkeys[next_monkey_index].receive_item(item)
    return monkeys
...
def solve_part_two(input_data):
    ...
    total_modulus = math.lcm(*[m.moduli for m in input_data])
    # total_modulus = 9699690
    print(total_modulus)
    monkeys = run_rounds(input_data, rounds=10000)
    items_touched = sorted([monkey.total_items_touched for monkey in monkeys])
    print(items_touched)
...
def main():
    ...
    parsed_data = parse(puzzle.input_data, nervous=False)
