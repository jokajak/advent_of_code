#!/usr/bin/env python
"""Solutions for AoC 10, 2022."""
# Created: 2022-12-10 00:00:05.546194

# Standard library imports
from aocd.models import Puzzle


def schedule_instruction(instruction_counter, instruction):
    match instruction:
        case "noop":
            return instruction_counter + 1
        case "addx":
            return instruction_counter + 2
...
def parse(input_data):
    ...
    for line in input_data.splitlines():
        ...
        if " " in line:
            ...
            cmd = line
        instruction_pointer = schedule_instruction(instruction_pointer, cmd)
        last_instruction = max(last_instruction, instruction_pointer)
        ret[instruction_pointer] = int(value)
    ret["last_instruction"] = last_instruction
    return ret


def run_program(instructions, last_instruction=None):
    """Run program.
...
def get_signal_strength(instructions, iteration):
    """Get signal strength.
    ...
    *during* the 20th cycle and every 40 cycles after that (that is,
    ...
    """
    # if iteration in instructions:
    #     print(instructions[iteration])
    # else:
    #     for i in range(iteration - 3, iteration + 3):
    #         if i in instructions:
    #             print(instructions[i])
    value = run_program(instructions, last_instruction=iteration)
...
def solve_part_one(input_data):
    ...
    *Unless*, that is, you can design a replacement for the device's video
    ...
    tube](https://en.wikipedia.org/wiki/Cathode-ray_tube) screen and
    simple CPU that are both driven by a precise *clock circuit*. The
    ...
    single register, `X`, which starts with the value `1`. It supports
...
def solve_part_two(input_data):
    ...
    pixels = []
    r = "c"
    for crt_pixel in range(0, 240):
        value = run_program(input_data, last_instruction=crt_pixel + 1)
        prev_value = run_program(input_data, last_instruction=crt_pixel)
        if (crt_pixel % 40) in range(value - 1, value + 2):
            new_pixel = "#"
        else:
            new_pixel = "."
        ...
        if r != "c":
            print(
                f"pixel={crt_pixel} X={value} sprites={range(value -1, value +1)} {new_pixel} prev_value={prev_value}"
            )
            r = input("continue?")
    output = []
    row = []
