#!/usr/bin/env python
"""AoC 2, 2022.
Created: 2022-12-02 14:08:51.903723
...
`A` for Rock, `B` for Paper, and `C` for Scissors. The second
...
def score_round(left, right):
    ...
    *outcome of the round* (0 if you lost, 3 if the round was a draw, and
    ...
    if left == "A":  # rock
        ...
        if right == "Y":  # paper
            round_score += 3
        elif right == "Z":  # scissors
            round_score += 6
    elif left == "C":  # scissors
        if right == "X":  # rock
            round_score += 6
        elif right == "Z":  # scissors
            round_score += 3
...
def solve_part_one(input_data):
    ...
    for line in input_data:
        ...
        score = score + score_round(opponent_play, my_play)
    answer = score
    return answer
